...
public class SectionHeader extends StreamReader {
	...
	public SectionHeader(byte[] data, int size, int offset,
			NameSectionHeader names) {
		this(data, size, offset, names.getSectionName(readInt(
	...
	public SectionHeader(byte[] data, int size, int offset, String name) {
		...
		case ELF.SHT_STRTAB:
			...
			break;
		case ELF.SHT_RELA:
			section = new RelocationSection(data, s_size, s_offset, s_entsize,
					true);
			break;
		case ELF.SHT_REL:
			section = new RelocationSection(data, s_size, s_offset, s_entsize,
