...
public class RelocationSection extends Section {
	...
	public RelocationSection(byte[] data, int s_size, int s_offset,
			int entrySize, boolean RELA) {
		super(data, s_size, s_offset);
		this.entrySize = entrySize;
		this.RELA = RELA;

		for (int index = 0; index < s_size / entrySize; index++) {
			entries.add(new RelocationSectionEntry(data, entrySize, s_offset
					+ entrySize * index, RELA));
		}
	}
