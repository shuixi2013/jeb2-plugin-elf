...
public class RelocationSectionEntry extends StreamReader {
    ...
    public void doRelocation(byte[] mem, int addrShift) {
        int A;
        if(RELA) {
            A = addend;
        }
        ...
        R_SYMBOL symbol = R_SYMBOL.EXTERNAL;
        if(symTabEntry.getBind() == ELF.STB_LOCAL && symTabEntry.getType() == ELF.STT_SECTION) {
            symbol = R_SYMBOL.LOCAL;
        }
        int newVal;
        byte[] newBytes;
        switch(type) {
            case ELF.R_MIPS_NONE:
                return;
            case ELF.R_MIPS_REL_32:
                if(symbol == R_SYMBOL.LOCAL) {
                    newVal = ELF.relocate(type, A, 32, 0, 0, symTabEntry.getValue(), 0, 0, 0, 0, 0, symbol);
                    newBytes = ByteBuffer.allocate(4).putInt(newVal).array();
                    System.arraycopy(newBytes, 0, mem, entryOffset, 4);
                }
                else if(symbol == R_SYMBOL.EXTERNAL) {
                    newVal = ELF.relocate(type, A, 32, 0, 0, symTabEntry.getValue(), 0,0,0,0,0, symbol);
                    newBytes = ByteBuffer.allocate(4).putInt(newVal).array();
                    System.arraycopy(newBytes, 0, mem, entryOffset, 4);
                }
