...
public class SectionHeaderTable extends StreamReader {
	...
	public SectionHeaderTable(byte[] data, int offset, short entrySize,
			short number, short nameTableIndex,
		...
		NameSectionHeader names = new NameSectionHeader(data, entrySize, offset
				+ nameTableIndex * entrySize);
		...
		for (int sh_index = 0; sh_index < number; sh_index++) {
			header = new SectionHeader(data, entrySize, offset + sh_index
					* entrySize, names);
			entries.add(header);
			if (header.getType() == ELF.SHT_RELA
					|| header.getType() == ELF.SHT_REL) {
		...
		for (int index = 0; index < entries.size(); index++) {
			...
			if (header.getLink() > entries.size() || header.getLink() < 0) {
				notifications.add(new UnitNotification(
						NotificationType.CORRUPTION,
						"sh_link points to a nonexistent section"));
			}
			if (header.getType() == ELF.SHT_DYNSYM
					|| header.getType() == ELF.SHT_SYMTAB) {
				int strtabIndex = header.getLink();
				nameTable = (StringTableSection) (entries.get(strtabIndex)
						.getSection());
				header.setNameTable(nameTable);
			}
			if (header.getType() == ELF.SHT_REL
					|| header.getType() == ELF.SHT_RELA) {
				((RelocationSection) header.getSection())
						.setSymbolTable(entries.get(header.getLink()));
			}
			if (header.getType() == ELF.SHT_DYNAMIC) {
				((DynamicSection) header.getSection()).setStringTable(entries
						.get(header.getLink()));
			}
		}
	}
