...
public class ELFUnit extends AbstractBinaryUnit implements ICodeObjectUnit,
		IInteractiveUnit {
	...
	public ELFUnit(String name, IInput input, IUnitProcessor unitProcessor,
	...
	public boolean process() {
		...
		for (SectionHeader header : elf.getSectionHeaderTable().getHeaders()) {
			if (header.getType() == SHT_DYNSYM
					|| header.getType() == SHT_SYMTAB) {
				...
				for (SymbolTableEntry entry : symtab.getEntries()) {
					...
					symbols.add(new SymbolInformation(symType, 0, (long) entry
							.getValue(), entry.getName(), (long) entry
							.getValue(), (long) entry.getValue(), entry
							.getSize()));
				}
			}
		...
		for (ProgramHeader header : elf.getProgramHeaderTable().getHeaders()) {
			if ((header.getType() & PT_LOAD) != 0
					&& header.getSizeInMemory() > 0) {
		...
		for (ISegmentInformation segment : segments) {
			...
			if (segment.getOffsetInMemory() + segment.getSizeInMemory() > maxAddr) {
				maxAddr = segment.getOffsetInMemory()
						+ segment.getSizeInMemory();
			}
		}
		if (maxAddr > Integer.MAX_VALUE || minAddr > Integer.MAX_VALUE) {
			throw new RuntimeException(String.format(
					"Can't pass IInput larger than Integer.MAX_VALUE", maxAddr));
		}
		processImage = new byte[(int) (maxAddr - minAddr)];
		for (ISegmentInformation segment : segments) {
			if (segment.getSizeInFile() > 0) {
				System.arraycopy(data, (int) segment.getOffsetInFile(),
						processImage,
						(int) (segment.getOffsetInMemory() - minAddr),
		...
		if (targetType != null) {
			target = unitProcessor.process(name, new BytesInput(processImage),
		...
		for (SectionHeader header : elf.getSectionHeaderTable().getHeaders()) {
			if (header.getSection() != null) {
				try {
					target = unitProcessor.process(header.getName(),
							new BytesInput(header.getSection().getBytes()),
							this, null, true);
					if (target != null) {
						reparseUnits.add(target);
					}
	...
	public String getDescription() {
		...
		desc.append(String.format("%-40s%-20s\n", "Magic:",
				header.getMagicString()));
		desc.append(String.format("%-40s%-20s\n", "Class:",
				header.getClassString()));
		desc.append(String.format("%-40s%-20s\n", "Data:",
				header.getDataString()));
		desc.append(String.format("%-40s%-20s\n", "Version:",
				header.getVersionString()));
		desc.append(String.format("%-40s%-20s\n", "OS/ABI:",
				header.getOSABIString()));
		desc.append(String.format("%-40s%-20s\n", "ABI version:",
				"" + header.getABIVersion()));
		desc.append(String.format("%-40s%-20s\n", "Type:",
				header.getTypeString()));
		desc.append(String.format("%-40s%-20s\n", "Machine:",
				header.getMachineString()));
		desc.append(String.format("%-40s%-20s\n", "Version:",
				"0x" + Integer.toHexString(header.getVersion())));
		desc.append(String.format("%-40s%-20s\n", "Entry point address:", "0x"
				+ Integer.toHexString(header.getEntryPoint())));
		desc.append(String.format("%-40s%-20s\n", "Start of program headers:",
				"0x" + Integer.toHexString(header.getPHOffset())));
		desc.append(String.format("%-40s%-20s\n", "Start of section headers:",
				"0x" + Integer.toHexString(header.getShoff())));
		desc.append(String.format("%-40s%-20s\n", "Flags:",
				"0x" + Integer.toHexString(header.getFlags())));
		desc.append(String.format("%-40s%-20s\n", "Size of this header:",
				header.getHeaderSize()));
		desc.append(String.format("%-40s%-20s\n", "Size of program headers:",
				header.getPHEntrySize()));
		desc.append(String.format("%-40s%-20s\n", "Number of program headers:",
				header.getPHNumber()));
		desc.append(String.format("%-40s%-20s\n", "Size of section headers:",
				header.getSHEntrySize()));
	...
	public IUnitFormatter getFormatter() {
		List<SectionHeader> sectionHeaders = elf.getSectionHeaderTable()
				.getHeaders();
		UnitFormatterAdapter formatter = new UnitFormatterAdapter();
		formatter.addDocumentPresentation(new AbstractUnitRepresentation(
				"Section Header Table", true) {
			@Override
			public IGenericDocument getDocument() {
				return new SectionHeaderTableDocument(elf
						.getSectionHeaderTable());
			}
		});

		formatter.addDocumentPresentation(new AbstractUnitRepresentation(
				"Program Header Table", false) {
			@Override
			public IGenericDocument getDocument() {
				return new ProgramHeaderTableDocument(elf
						.getProgramHeaderTable());
			}
		});
		for (SectionHeader section : sectionHeaders) {
			...
			case SHT_STRTAB:
				formatter
						.addDocumentPresentation(new AbstractUnitRepresentation(
								section.getName(), false) {
							@Override
							public IGenericDocument getDocument() {
	...
	public String locationToAddress(IInputLocationInformation location) {
		if (!(location instanceof FileInputLocationInformation)) {
			throw new IllegalArgumentException(
					"Unrecognized IInputLocationInformation implementor");
		}
